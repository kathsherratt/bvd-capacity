...
#'
#' A copy of `R/lib/gemini.R` from bvd-sitreps, kept as a copy rather than
#' sourced from there so that this repository depends on bvd-sitreps' data
#' and never on its code. Fixes to retry, quota, budget or backend handling
#' should be made in both.
#'
#' Every call here goes through `gemini()`: temperature 0 on the API, a
#' response schema, retries that wait out per-minute limits, a clean stop on
#' daily quota, exhausted billing, `GEMINI_BUDGET_USD` or the Antigravity plan
#' quota, and a ledger row per call.
#'
#' Two routes, chosen by `GEMINI_BACKEND`: `api` (billed per token to the key's
#' project) or `agy` (the Antigravity CLI on a Google AI Pro or Ultra plan, no
#' per-call cost). See the agy section below for how they differ.
#'
#' The extraction model is pinned in `GEMINI_MODEL_EXTRACT`. See
#' R/01_facilities.R for why it is the one it is.

GEMINI_ENDPOINT <- "https://generativelanguage.googleapis.com/v1beta/models"

GEMINI_MODEL_EXTRACT <- Sys.getenv("GEMINI_MODEL_EXTRACT",
...
#'
#' Classed so that extraction loops can tell "no more calls today" from
...
gemini <- function(parts, schema, model = GEMINI_MODEL_EXTRACT,
